...
function TagButton({ label, isActive, onClick, count }: { label: string, isActive: boolean, onClick: () => void, count?: number }) {
  return (
    <button
      onClick={onClick}
      className={`group flex w-full items-center gap-1.5 rounded border px-2 py-1.5 text-left transition-all ${
        isActive ? "border-border-accent bg-bg-surface" : "border-transparent hover:border-border-accent hover:bg-bg-surface"
      }`}
    >
      <div className={`h-1.25 w-1.25 shrink-0 rotate-45 transition-colors ${isActive ? "bg-gold" : "bg-border-accent"}`} />
      <span className={`flex-1  capitalize text-xs transition-colors ${isActive ? "text-gold" : "text-text-mid group-hover:text-gold"}`}>
        {label}
      </span>
      {count != null && <span className="text-[10px] text-text-dim">{count}</span>}
    </button>
  );
}
      ...
      <div className="relative flex flex-col items-center pb-6 
        ...
        <div className="flex gap-5">
          <div className="flex flex-col items-center">
            <span className="text-sm font-medium text-gold">{user?.following_count}</span>
            <span className="text-[10px] uppercase tracking-[1px] text-text-dim">Following</span>
          </div>
          <div className="flex flex-col items-center">
            <span className="text-sm font-medium text-gold">{user?.followers_count}</span>
            <span className="text-[10px] uppercase tracking-[1px] text-text-dim">Followers</span>
          </div>
        </div>
      </div>
      <div>
        ...
        <div className="flex flex-col gap-1">
          {isLoading && (
            <div className="text-xs text-text-dim px-2 py-1.5">Gathering tags...</div>
          )}

          {isError && (
            <div className="text-xs text-red-900/50 px-2 py-1.5">Failed to read magical signs.</div>
          )}

          <TagButton label="All" isActive={selectedTag === null} onClick={() => setSelectedTag(null)} />
